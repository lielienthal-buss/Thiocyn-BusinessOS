...
function IntegrationSecretsSection({ brandSlug }: { brandSlug: string }) {
  ...
  if (loading) return <div className="text-sm text-slate-600 py-4">Lade Integrations...</div>;

  return (
    <div className="bg-white ring-1 ring-slate-200 rounded-2xl p-4">
      <h3 className="text-sm font-semibold text-slate-900 mb-1">Integration Secrets</h3>
      <p className="text-xs text-slate-600 mb-4">
        API Keys werden verschlüsselt gespeichert. Werte werden nie angezeigt — nur der Status.
      </p>

      {message && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-2 text-xs text-emerald-700 mb-3">
          {message}
        </div>
      )}
      ...
      {INTEGRATIONS.map(({ integration, keys, label }) => (
        <div key={integration} className="mb-4 last:mb-0">
          <h4 className="text-[10px] font-semibold uppercase tracking-wider text-slate-600 mb-2">{label}</h4>
              ...
              return (
                <div key={fullKey} className="flex items-center gap-2">
                  <div className="flex-1">
                    <label className="block text-xs text-slate-600 mb-0.5">{keyName}</label>
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 bg-white ring-1 ring-slate-200 text-slate-900 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 placeholder-slate-500"
                      />
                      <button
                        onClick={() => handleSave(integration, keyName)}
                        disabled={isSaving || !inputValues[fullKey]?.trim()}
                        className="px-3 py-1.5 bg-indigo-600 text-white rounded-lg text-xs font-semibold hover:bg-indigo-700 disabled:opacity-30 transition-colors"
                      >
                        {isSaving ? '...' : 'Save'}
                      </button>
                      {isConfigured && (
                        <button
                          ...
                          className="px-2 py-1.5 text-rose-700 hover:bg-rose-50 rounded-lg text-xs transition-colors"
                  ...
                  <span className={`text-xs font-semibold mt-4 ${isConfigured ? 'text-emerald-700' : 'text-slate-500'}`}>
                    {isConfigured ? '✓' : '—'}
                  </span>
                </div>
              );
  ...
  return (
    <div className="flex gap-4 h-full">
      ...
      <div className="w-48 shrink-0 flex flex-col gap-1">
        <h3 className="text-sm font-semibold text-slate-900 mb-3">Brands</h3>
        {brands.map((brand) => (
          <button
            key={brand.slug}
      ...
      <div className="flex-1 overflow-y-auto">
        {activeBrand ? (
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-4 py-2 bg-slate-50 text-slate-900 ring-1 ring-slate-200 rounded-xl text-sm font-medium hover:bg-slate-100"
                >
                  Abbrechen
                </button>
                <button
            ...
            <div className="bg-slate-50 ring-1 ring-slate-200 rounded-xl p-3 text-xs text-slate-600">
            ...
            <div className="bg-white ring-1 ring-slate-200 rounded-2xl p-4">
              <h3 className="text-sm font-semibold text-slate-900 mb-3">Shopify</h3>
              <div className="flex flex-col gap-3">
                <div>
                  ...
                  <input
                    className="w-full bg-white ring-1 ring-slate-200 text-slate-900 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 placeholder-slate-500"
            ...
            {/* Section 2 — Meta Ads */}
            <div className="bg-white ring-1 ring-slate-200 rounded-2xl p-4">
              <h3 className="text-sm font-semibold text-slate-900 mb-3">Meta Ads</h3>
              <div className="flex flex-col gap-3">
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1">
                    Ad Account ID
            ...
            <div className="bg-white ring-1 ring-slate-200 rounded-2xl p-4">
              <h3 className="text-sm font-semibold text-slate-900 mb-3">Other Platforms</h3>
              <div className="flex flex-col gap-3">
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1">
            ...
            <div className="bg-white ring-1 ring-slate-200 rounded-2xl p-4">
              <h3 className="text-sm font-semibold text-slate-900 mb-3">Communication</h3>
              <div className="flex flex-col gap-3">
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1">
                    CS Email
            ...
            {/* Notes */}
            <div className="bg-white ring-1 ring-slate-200 rounded-2xl p-4">
              <h3 className="text-sm font-semibold text-slate-900 mb-3">Notizen</h3>
              <textarea
                className="w-full bg-white ring-1 ring-slate-200 text-slate-900 rounded-xl px-3 py-2 text-sm h-24 focus:outline-none focus:ring-2 focus:ring-indigo-500 placeholder-slate-500"
      ...
      {/* Toast */}
      {showToast && (
        <div className="fixed bottom-4 right-4 bg-emerald-600 text-white px-4 py-2 rounded-xl text-sm shadow-lg z-50">
          Gespeichert ✓
        </div>
      )}
    </div>

...
const ScoreBar: React.FC<{ label: string; value: number; weight: string }> = ({ label, value, weight }) => (
  <div>
    ...
    <p className="text-[9px] text-gray-400 mt-0.5 text-right">weight {weight}</p>
  </div>
);
...
const InternCard: React.FC<{
  ...
  return (
    <div className={`bg-white border rounded-2xl overflow-hidden transition-all ${scoreRing(score.overall)} ring-2`}>
      {/* Header */}
      <div
      ...
      >
        ...
        <div className={`w-14 h-14 rounded-2xl flex flex-col items-center justify-center shrink-0 ${
          score.overall >= 80 ? 'bg-green-50' : score.overall >= 60 ? 'bg-amber-50' : 'bg-red-50'
        }`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500 font-medium">{intern.role}</p>
          <div className="flex items-center gap-3 mt-1.5 flex-wrap">
            <span className="text-[10px] text-gray-500">
              <span className="font-bold text-gray-700">{score.raw.done}</span>/{score.raw.total} tasks
            </span>
            <span className="text-[10px] text-gray-500">
              <span className="font-bold text-green-600">{score.raw.onTime}</span> on-time
            </span>
            {open.length > 0 && (
        ...
        <div className="hidden md:flex flex-col gap-1 w-28 shrink-0">
          {[
            ...
            <div key={d.label} className="flex items-center gap-1.5">
              <span className="text-[9px] text-gray-400 w-14 text-right">{d.label}</span>
      ...
      {expanded && (
        <div className="border-t border-gray-100 px-5 py-4 space-y-4">
            ...
            <div>
              ...
              <div className="space-y-1.5">
                {done.filter(t => t.quality_score == null).map(task => (
                  <div key={task.id} className="flex items-center gap-2 text-xs bg-amber-50 border border-amber-100 rounded-lg px-3 py-2">
                    <span className="flex-1 text-gray-700 font-medium truncate">{task.title}</span>
                    {task.brand && (
          ...
          {open.length > 0 && (
            <div>
              <p className="text-xs font-bold text-gray-500 mb-2">Open tasks</p>
              <div className="space-y-1">
                {open.map(task => (
                  <div key={task.id} className="flex items-center gap-2 text-xs bg-gray-50 rounded-lg px-3 py-1.5">
...
const RateTaskModal: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        <h3 className="font-black text-gray-900 mb-1">Rate Task</h3>
        <p className="text-xs text-gray-500 mb-5 truncate">{task.title}</p>
        ...
        <div className="mb-5">
            ...
            {[1, 2, 3, 4, 5].map(v => (
              <button
                ...
                className={`flex-1 py-2 rounded-xl text-sm font-black border-2 transition-all ${
                  quality === v ? 'border-primary-500 bg-primary-50 text-primary-700' : 'border-gray-200 text-gray-500 hover:border-gray-300'
                }`}
              >
                {v}
              </button>
          ...
          <p className="text-xs text-gray-500 mt-1.5 text-center">{QUALITY_LABELS[quality]}</p>
        </div>
        ...
        <div className="mb-6">
            ...
            {[1, 2, 3].map(v => (
              <button
                ...
                className={`flex-1 py-2.5 rounded-xl text-sm font-black border-2 transition-all ${
                  impact === v ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-500 hover:border-gray-300'
                }`}
              >
                {v}
              </button>
          ...
          <p className="text-xs text-gray-500 mt-1.5 text-center">{IMPACT_LABELS[impact]}</p>
        </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div>
          <h2 className="text-lg font-black text-gray-900">Team Performance</h2>
        ...
        <div className="ml-auto flex items-center gap-2">
          {totalUnrated > 0 && (
            <span className="px-3 py-1.5 bg-amber-50 border border-amber-200 text-amber-700 text-xs font-bold rounded-full">
      ...
      <div className="grid grid-cols-3 gap-3 text-center">
        {[
          { label: 'High Performer', range: '80–100', color: 'bg-green-50 border-green-200 text-green-700' },
          { label: 'On Track', range: '60–79', color: 'bg-amber-50 border-amber-200 text-amber-700' },
          { label: 'Needs Attention', range: '0–59', color: 'bg-red-50 border-red-200 text-red-600' },
        ].map(l => (
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-gray-500 font-semibold">No team tasks assigned yet.</p>
          <p className="text-gray-400 text-sm mt-1">Assign tasks via 👥 Team Board to start tracking performance.</p>
        </div>
      ) : (

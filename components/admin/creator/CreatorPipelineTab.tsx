    ...
    <>
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-colors ${
            showUntaggedOnly
              ...
              : 'bg-surface-800/60 border-white/[0.06] text-slate-400 hover:text-slate-200'
          }`}
        >
      ...
      {selectedIds.size > 0 && (
        <div className="flex items-center gap-3 bg-primary-500/10 border border-primary-500/20 rounded-xl p-3">
          <span className="text-sm font-bold text-primary-400">{selectedIds.size} selected</span>
          <select
            value={bulkBrand}
          ...
          <button
            ...
            className="px-4 py-1.5 bg-primary-600 hover:bg-primary-700 text-white rounded-lg text-xs font-bold transition-colors disabled:opacity-50"
          >
        ...
        <div className="bg-surface-800/60 rounded-xl border border-white/[0.06] overflow-hidden backdrop-blur-sm">
          <table className="w-full text-sm">
              ...
              <tr className="bg-surface-900/60 border-b border-white/[0.06]">
                <th className="px-3 py-3 w-10">
                  <input type="checkbox" checked={selectedIds.size === filtered.length && filtered.length > 0}
                    ...
                    className="rounded border-white/20 bg-surface-900/60 text-primary-500 focus:ring-primary-500/30" />
                </th>
                ...
                <th className="px-4 py-3 text-left text-[10px] font-black uppercase tracking-widest text-slate-500">Slug</th>
                ...
                <th className="px-4 py-3 text-left text-[10px] font-black uppercase tracking-widest text-slate-500">Affiliate Code</th>
            ...
            <tbody className="divide-y divide-white/[0.06]">
              {filtered.map(c => (
                <tr key={c.id} className={`hover:bg-white/[0.03] transition-colors ${selectedIds.has(c.id) ? 'bg-primary-500/5' : ''}`}>
                  ...
                  <td className="px-4 py-3">
                    {c.brand_slug ? (
                      <span className="text-xs font-mono text-emerald-400">{c.brand_slug}</span>
                    ) : (
                      <span className="text-xs font-mono text-amber-400/60">untagged</span>
                    )}
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold ${TIER_COLORS[c.tier ?? 'gifting']}`}>
                      {c.tier ?? 'gifting'}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    {c.affiliate_code ? (
                      <div className="flex items-center gap-1.5">
                        <span className="text-xs font-mono text-slate-300">{c.affiliate_code}</span>
                        <button

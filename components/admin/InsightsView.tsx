...
const InsightsView: React.FC = () => {
  ...
  const psychometricsAverages = applications.reduce((acc, app) => {
  ...
  const bigFiveAverages = Object.entries(psychometricsAverages).map(([trait, data]) => ({
    trait,
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-12 gap-6 animate-[fadeIn_0.5s_ease-out]">
      ...
      <div className="md:col-span-4 p-8 rounded-[2.5rem] flex flex-col justify-center bg-gray-900/30 backdrop-blur-2xl border border-white/20">
        <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Total Applicants</p>
      ...
      <div className="md:col-span-8 p-8 rounded-[2.5rem] bg-gray-900/30 backdrop-blur-2xl border border-white/20">
        <h3 className="text-xs font-black uppercase tracking-widest text-gray-400 mb-6">V2 Hiring Funnel</h3>
        <div className="space-y-4">
          {[
            { label: 'Applied', count: total },
            { label: 'Task Submitted', count: stageCounts['task_submitted'] || 0 },
            ...
            <div key={i} className="space-y-1">
              ...
              <div className="w-full h-3 bg-gray-100 dark:bg-slate-800 rounded-full overflow-hidden">
                <div className="h-full bg-primary-600 transition-all duration-1000" style={{ width: total > 0 ? `${(step.count / total) * 100}%` : '0%' }}></div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="md:col-span-6 p-8 rounded-[2.5rem] bg-gray-900/30 backdrop-blur-2xl border border-white/20">
        <h3 className="text-xs font-black uppercase tracking-widest text-gray-400 mb-8">Avg. Personality Profile</h3>
          ...
          {bigFiveAverages.map(({ trait, average }) => (
            <div key={trait}>
              <div className="flex justify-between mb-1">
                ...
                <span className="text-sm font-bold text-gray-900 dark:text-white">{average}%</span>
              </div>
              <div className="w-full bg-gray-200 dark:bg-slate-700 rounded-full h-2.5">
                <div className={`${traitColors[trait] || 'bg-gray-500'} h-2.5 rounded-full`} style={{ width: `${average}%` }}></div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="md:col-span-6 p-8 rounded-[2.5rem] grid grid-cols-2 gap-4 bg-gray-900/30 backdrop-blur-2xl border border-white/20">
        {Object.entries(stageCounts).map(([stage, count]) => (
          <div key={stage} className="p-4 rounded-2xl bg-white/30 dark:bg-slate-900/30 border border-white/10">
            <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">{stage}</p>

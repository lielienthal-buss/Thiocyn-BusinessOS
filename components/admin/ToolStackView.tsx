...
const ToolStackView: React.FC<Props> = ({ isAdmin }) => {
  ...
  if (loading) return <div className="text-sm text-slate-500 py-8 text-center">Laden…</div>;
  ...
  return (
    <div className="space-y-6 animate-[fadeIn_0.3s_ease-out]">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-xl font-black text-white tracking-tight">Tool Stack</h2>
          <p className="text-sm text-slate-500 mt-0.5">Alle Tools, Kosten & Einsparpotenziale</p>
        </div>
        {isAdmin && (
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="px-4 py-3 rounded-xl bg-surface-900/60 border border-white/[0.06]">
          <div className="text-[9px] font-black uppercase tracking-[0.25em] text-slate-500">Monatliche Kosten</div>
        ...
        <div className="px-4 py-3 rounded-xl bg-surface-900/60 border border-white/[0.06]">
          <div className="text-[9px] font-black uppercase tracking-[0.25em] text-slate-500">Aktive Tools</div>
          <div className="text-xl font-black text-white mt-1">
      ...
      <div className="flex flex-wrap gap-2">
        {CATEGORIES.map(cat => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider transition-all ${
              filter === cat ? 'bg-white/20 text-white' : 'bg-white/[0.05] text-slate-500 hover:bg-white/[0.08]'
            }`}
          >
            {cat}
          </button>
        ...
        {['All', 'active', 'review', 'cancelling', 'cancelled'].map(s => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider transition-all ${
              statusFilter === s ? 'bg-white/20 text-white' : 'bg-white/[0.05] text-slate-500 hover:bg-white/[0.08]'
            }`}
          >
            {s === 'All' ? 'Alle Status' : s}
          </button>
      ...
      <div className="rounded-xl border border-white/[0.06] overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-surface-900/60 border-b border-white/[0.06]">
              <th className="text-left px-4 py-3 text-[9px] font-black uppercase tracking-[0.25em] text-slate-500">Tool</th>
              <th className="text-left px-4 py-3 text-[9px] font-black uppercase tracking-[0.25em] text-slate-500">Kategorie</th>
              <th className="text-left px-4 py-3 text-[9px] font-black uppercase tracking-[0.25em] text-slate-500">Status</th>
              <th className="text-right px-4 py-3 text-[9px] font-black uppercase tracking-[0.25em] text-slate-500">€/Monat</th>
              <th className="text-left px-4 py-3 text-[9px] font-black uppercase tracking-[0.25em] text-slate-500">Owner</th>
              <th className="text-left px-4 py-3 text-[9px] font-black uppercase tracking-[0.25em] text-slate-500">Notiz</th>
              {isAdmin && <th className="px-4 py-3" />}
            </tr>
          </thead>
          <tbody className="divide-y divide-white/[0.04]">
            {filtered.map(tool => (
              <tr key={tool.id} className={`hover:bg-white/[0.03] transition-colors ${tool.status === 'cancelled' ? 'opacity-40' : ''}`}>
                ...
                <td className="px-4 py-3 text-xs text-slate-500">{tool.category}</td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3 text-right font-mono text-xs text-white">
                  {tool.billing_cycle === 'free' ? <span className="text-slate-500">—</span>
                    : tool.monthly_cost !== null ? `${tool.monthly_cost.toFixed(0)} €`
                    : <span className="text-yellow-400 font-black">?</span>}
                </td>
                <td className="px-4 py-3 text-xs text-slate-500">{tool.owner ?? '—'}</td>
                <td className="px-4 py-3 text-xs text-slate-500 max-w-[200px] truncate">{tool.notes ?? ''}</td>
                {isAdmin && (
                  <td className="px-4 py-3">
                    <div className="flex gap-1 justify-end">
                      <button onClick={() => openEdit(tool)} className="px-2 py-1 text-[10px] text-slate-500 hover:text-white hover:bg-white/[0.08] rounded transition-colors">Edit</button>
        ...
        {filtered.length === 0 && (
          <div className="text-center py-8 text-sm text-slate-500">Keine Tools in dieser Kategorie.</div>
        )}
      </div>
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setShowAdd(false)}>
          <div className="bg-surface-800 border border-white/[0.08] rounded-2xl p-6 w-full max-w-md space-y-4 shadow-2xl" onClick={e => e.stopPropagation()}>
            <h3 className="font-black text-white">{editing ? 'Tool bearbeiten' : 'Tool hinzufügen'}</h3>
            {([
              ...
              <div key={field.key}>
                <label className="text-[10px] font-black uppercase tracking-wider text-slate-500 block mb-1">{field.label}</label>
                <input
                  type={field.type}
                  ...
                  className="w-full border border-white/[0.10] rounded-lg px-3 py-2 bg-white/[0.04] text-slate-100 text-sm focus:outline-none focus:border-white/20"
                />
              </div>
            ))}
            <div>
              <label className="text-[10px] font-black uppercase tracking-wider text-slate-500 block mb-1">Status</label>
              <select
            ...
            <div>
              <label className="text-[10px] font-black uppercase tracking-wider text-slate-500 block mb-1">Billing</label>
              <select
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button onClick={() => { setShowAdd(false); setEditing(null); setForm({}); }} className="px-4 py-2.5 border border-white/[0.06] text-sm text-slate-400 rounded-lg hover:bg-white/[0.05] transition-colors">Abbrechen</button>
            </div>
          </div>
        </div>
      )}

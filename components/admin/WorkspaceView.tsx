              ...
              <div key={mail.id} className="bg-surface-800/60 border border-white/[0.06] rounded-2xl p-4 backdrop-blur-sm space-y-3">
                ...
                {mail.ai_analysis && expandedId === mail.id && (
                  <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl px-4 py-3 space-y-1.5">
                    <div className="flex items-center justify-between">
                      <p className="text-xs font-semibold text-blue-400 uppercase tracking-wide">{t('mt.analysisTitle')}</p>
                      <button onClick={() => setExpandedId(null)} className="text-blue-500 hover:text-blue-300 text-xs">✕</button>
                    </div>
                    {mail.ai_analysis.summary && <p className="text-xs text-blue-300">{mail.ai_analysis.summary}</p>}
                    {mail.ai_analysis.action_reason && <p className="text-xs text-blue-400">💡 {mail.ai_analysis.action_reason}</p>}
                  </div>
                )}
                {mail.ai_analysis && expandedId !== mail.id && (
                  <button onClick={() => setExpandedId(mail.id)} className="text-xs text-blue-400 hover:text-blue-300 font-medium">{t('mt.showAnalysis')}</button>
                )}
                ...
                <div className="flex flex-wrap items-center gap-2 pt-1 border-t border-white/[0.06]">
                  ...
                  <button onClick={() => updateStatus(mail.id, 'new')} disabled={mail.status === 'new'}
                    className="px-3 py-1.5 text-xs font-semibold bg-blue-500/15 text-blue-400 rounded-lg hover:bg-blue-500/25 disabled:opacity-40 transition-colors">
                    {t('mt.markNew')}
                  </button>
            ...
            {req.status !== 'granted' && req.status !== 'denied' && (
              <div className="flex gap-2 pt-1 border-t border-white/[0.06]">
                ...
                <button onClick={() => updateStatus(req.id, 'granted')}
                  className="px-3 py-1 text-xs font-semibold bg-emerald-500/15 text-emerald-400 rounded-lg hover:bg-emerald-500/25 transition-colors">{t('zt.markGranted')}</button>
                <button onClick={() => updateStatus(req.id, 'denied')}
                  className="px-3 py-1 text-xs font-semibold bg-red-500/15 text-red-400 rounded-lg hover:bg-red-500/25 transition-colors">{t('zt.markDenied')}</button>
              </div>
            )}
          </div>
        ))}
